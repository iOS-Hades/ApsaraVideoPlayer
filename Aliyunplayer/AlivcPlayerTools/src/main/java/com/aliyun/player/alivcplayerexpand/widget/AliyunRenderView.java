...
import com.aliyun.player.alivcplayerexpand.util.AliYunPlayerUtil;
...
public class AliyunRenderView extends FrameLayout {
    ...
    AliYunPlayerUtil aliYunPlayerUtil;
    private void initPlayer(){
        mAliPlayer = AliPlayerFactory.createAliPlayer(mContext.getApplicationContext());
        aliYunPlayerUtil=new AliYunPlayerUtil(mContext);
        initPlayerListener();
    }
    ...
    private void initPlayerListener(){
        ...
        mAliPlayer.setOnVerifyStsCallback(new OnAVPVerifyStsCallback(this));
...
//            mAliPlayer.setSpeed(speed);
            aliYunPlayerUtil.changeSpeed(mAliPlayer, speed);
        }
    }
    ...
    private static class OnAVPVerifyStsCallback implements IPlayer.OnVerifyStsCallback{
    ...
    private IPlayer.OnVerifyStsCallback mOnVerifyStsCallback;

    public void setOnVerifyStsCallback(IPlayer.OnVerifyStsCallback listener){
        this.mOnVerifyStsCallback = listener;
    }

    private IPlayer.StsStatus onVerifySts(StsInfo stsInfo) {
        if(mOnVerifyStsCallback != null){
            return mOnVerifyStsCallback.onVerifySts(stsInfo);
        }
        return IPlayer.StsStatus.Valid;
    }
}
